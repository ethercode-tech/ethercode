...
const BASE_MESSAGE =
  "Hola ÉtherCode! Quiero un Empleado Digital y me gustaría una propuesta rápida.";
...
export default function ChatWhatsapp() {
  ...
  useEffect(() => {
    const t1 = setTimeout(() => setShowTooltip(true), 6500);
    const t2 = setTimeout(() => setShowTooltip(false), 15000);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
    };
  ...
  return (
    // Este wrapper evita “bleed” del blur fuera del viewport
    <div className="fixed right-5 bottom-6 z-50" style={{ transform: "translateZ(0)" }}>
      {/* Aura: más chica, sin blur exagerado */}
      <div
        aria-hidden
        className="pointer-events-none absolute -right-6 -bottom-6 z-[-1] w-24 h-24 rounded-full opacity-25"
        style={{
          background:
            "radial-gradient(circle at 50% 50%, rgba(0,245,212,.35), rgba(199,125,255,.18), transparent 65%)",
          filter: "blur(18px)",
        }}
      />

      {/* Tooltip (ABSOLUTE, no fixed) */}
      {showTooltip && !showPanel && (
        <motion.div
          ...
          transition={{
            duration: reduceMotion ? 0.2 : 2.0,
            repeat: reduceMotion ? 0 : Infinity,
            ease: "easeInOut",
          }}
          className="absolute right-[72px] bottom-[58px] max-w-[240px] select-none text-[13px]
                     rounded-2xl px-3 py-2 text-black bg-white shadow-lg border border-black/5
                     pointer-events-none"
        >
          ¿Querés que te armemos una propuesta por WhatsApp?
          <div
      ...
      >
        ...
        <span
          className="pointer-events-none absolute -inset-1 rounded-full opacity-0 group-hover:opacity-100 transition"
          style={{
            background:
              "radial-gradient(60% 60% at 50% 50%, rgba(37,211,102,.55), transparent 60%)",
            filter: "blur(10px)",
          }}
        />
      </button>
      ...
      {showPanel && (
        <div
          ...
          className="absolute right-0 bottom-16 w-[290px] p-4 rounded-2xl
                     bg-[rgba(10,17,40,0.78)] backdrop-blur-md border border-white/10 shadow-2xl
                     overflow-hidden"
        >
          <h4 className="text-white font-semibold text-sm">Hablemos ahora</h4>
          <p className="text-white/70 text-xs mt-1">
            Te respondemos rápido y te decimos qué conviene para tu caso, sin vueltas.
          </p>
